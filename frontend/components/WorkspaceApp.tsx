...
type VerificationStatus = "verified" | "tampered" | "unverified" | "error";
...
function extractModelJson(payload: unknown): unknown | null {
  if (!payload || typeof payload !== "object") {
    return null;
  }

  if ("modelJson" in payload) {
    return (payload as { modelJson?: unknown }).modelJson ?? null;
  }

  if ("analysis" in payload) {
    const analysis = (payload as { analysis?: unknown }).analysis;
    if (analysis && typeof analysis === "object" && "modelJson" in analysis) {
      return (analysis as { modelJson?: unknown }).modelJson ?? null;
    }
  }
...
function extractVerificationMeta(payload: unknown): { status: VerificationStatus; lastVerifiedAt: string | null } {
  if (!payload || typeof payload !== "object") {
    return { status: "unverified", lastVerifiedAt: null };
  }
  const source =
    "analysis" in payload && (payload as { analysis?: unknown }).analysis && typeof (payload as { analysis?: unknown }).analysis === "object"
      ? ((payload as { analysis: Record<string, unknown> }).analysis as Record<string, unknown>)
      : (payload as Record<string, unknown>);

  const rawStatus = (
    (source as { verificationStatus?: unknown }).verificationStatus ??
  ...
  const normalized =
    typeof rawStatus === "string"
      ? rawStatus.toLowerCase() === "verified"
        ? "verified"
        : rawStatus.toLowerCase() === "tampered"
          ? "tampered"
  ...
  const lastVerifiedAtRaw = (source as { lastVerifiedAt?: unknown }).lastVerifiedAt;
  const lastVerifiedAt = typeof lastVerifiedAtRaw === "string" ? lastVerifiedAtRaw : null;

  return { status: normalized, lastVerifiedAt };
}

function verificationBadgeClass(status: VerificationStatus) {
  if (status === "verified") return "badge-primary";
  if (status === "tampered") return "badge-danger";
...
export default function WorkspaceApp({ initialAnalysisId = null, initialReadOnlyMode = false }: WorkspaceAppProps) {
  const [isHydrated, setIsHydrated] = useState(false);
  const { isDark: darkMode, toggleTheme } = useTheme();
  const analysisId = initialAnalysisId;
  const readOnlyMode = initialReadOnlyMode;
  const [analysisVerificationStatus, setAnalysisVerificationStatus] = useState<VerificationStatus>("unverified");
  const [analysisLastVerifiedAt, setAnalysisLastVerifiedAt] = useState<string | null>(null);
  const [analysisVerifying, setAnalysisVerifying] = useState(false);
  const [analysisVerifyError, setAnalysisVerifyError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadAnalysis() {
      ...
      setUploadMessage(`Loading analysis ${analysisIdValue}...`);
      try {
        const response = await fetch(`/api/analyses/${encodeURIComponent(analysisIdValue)}`, { cache: "no-store" });
        const payload = (await response.json()) as unknown;
        if (!response.ok) {
          const message =
            payload && typeof payload === "object" && "error" in payload
              ? String((payload as { error?: unknown }).error ?? `Failed to load analysis ${analysisIdValue}.`)
              : `Failed to load analysis ${analysisIdValue}.`;
          throw new Error(message);
        }

        const modelJson = extractModelJson(payload);
        if (!modelJson) {
          throw new Error("Analysis response does not include modelJson.");
        }
        const verificationMeta = extractVerificationMeta(payload);
        ...
        loadRawInput(modelJson);
        setAnalysisVerificationStatus(verificationMeta.status);
        setAnalysisLastVerifiedAt(verificationMeta.lastVerifiedAt);
        setAnalysisVerifyError(null);
        setUploadState("success");
        setUploadMessage(
          readOnlyMode
            ? `Loaded analysis ${analysisIdValue}. Workspace is read-only.`
            : `Loaded analysis ${analysisIdValue}.`,
        );
      } catch (error) {
        if (cancelled) {
          return;
        ...
        setAnalysisVerifyError(error instanceof Error ? error.message : "Could not load analysis metadata.");
        setUploadMessage(error instanceof Error ? error.message : `Could not load analysis ${analysisIdValue}.`);
      }
    }
  ...
  }, [analysisId, loadRawInput, readOnlyMode]);
  ...
  function handleReportElementFocus(elementId: string, elementType: CostReportElementType) {
    if (elementType === "load_bearing_wall" || elementType === "partition_wall") {
      handleIssueSelection({ type: "wall", id: elementId });
      return;
    }

    if (elementType === "slab") {
      handleIssueSelection({ type: "slab", id: elementId });
      return;
    }

    if (elementType === "column") {
      const column = derivedColumns.find((entry) => entry.id === elementId);
      if (!column) {
        return;
      }
      selectEntity({ type: "column", id: elementId });
      setIssueFocusPoint(column.position);
      setIssueFocusToken((current) => current + 1);
      return;
    }

    if (elementType === "beam") {
      const roomId = elementId.startsWith("beam_") ? elementId.slice("beam_".length) : "";
      if (roomId && scene.rooms.some((room) => room.id === roomId)) {
        handleIssueSelection({ type: "slab", id: roomId });
      }
    }
  }
  ...
  async function handleVerifyAnalysisAgain() {
    if (!analysisId || !readOnlyMode) {
      return;
    }
    setAnalysisVerifying(true);
    setAnalysisVerifyError(null);
    try {
      ...
      if (!response.ok) {
        const message =
          payload && typeof payload === "object" && "error" in payload
            ? String((payload as { error?: unknown }).error ?? "Verification failed.")
            : "Verification failed.";
        throw new Error(message);
      }
      const meta = extractVerificationMeta(payload);
      setAnalysisVerificationStatus(meta.status);
      setAnalysisLastVerifiedAt(meta.lastVerifiedAt);
      setUploadMessage(`Verification refreshed for analysis ${analysisId}.`);
      setUploadState("success");
    } catch (error) {
      setAnalysisVerifyError(error instanceof Error ? error.message : "Verification failed.");
    } finally {
      setAnalysisVerifying(false);
    }
  }
  ...
  return (
    ...
    >
      <div className="app-inner" style={{ maxWidth: 1680, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 20, flexWrap: "wrap" }}>
              ...
              <div className="header-quick-links" style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                ...
                <Link href="/analysis" className="btn btn-default">Analysis</Link>
                <button
                  type="button"
                  onClick={toggleTheme}
              ...
              {/* Controls */}
            <div className="header-controls header-scene-actions" style={{ display: "flex", flexWrap: "wrap", gap: 10, width: "100%" }}>
                {!readOnlyMode ? (
                  <>
                <label className="btn btn-default" style={{ cursor: "pointer" }}>
                  <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" style={{ flexShrink: 0 }}>
                    <path d="M14 3v4a1 1 0 001 1h4M8 13h8M8 17h8M8 9h2M14 3H7a2 2 0 00-2 2v14a2 2 0 002 2h10a2 2 0 002-2V8l-5-5z" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  Upload JSON
                ...
                >
                  ...
                  <div className="issue-entry info" style={{ maxWidth: 780, display: "flex", flexDirection: "column", gap: 8 }}>
                    <p style={{ margin: 0 }}>
                      Read-only mode is active for analysis {analysisId ?? "-"}. Scene modification actions are disabled.
                    </p>
                    <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8 }}>
                      <span className={`badge ${verificationBadgeClass(analysisVerificationStatus)}`}>
                        {verificationLabel(analysisVerificationStatus)}
                      </span>
                      <span style={{ fontSize: 12, color: "var(--fg-secondary)" }}>
                        {analysisLastVerifiedAt
                          ? `Last verified: ${new Date(analysisLastVerifiedAt).toLocaleString("en-IN")}`
                          : "Last verified: Never"}
                      </span>
                      <button
                        type="button"
                        className="btn btn-default"
                        onClick={handleVerifyAnalysisAgain}
                        disabled={analysisVerifying || !analysisId}
                        style={{ minHeight: 28, padding: "4px 10px", fontSize: 11 }}
                      >
                        {analysisVerifying ? "Verifying..." : "Verify Again"}
                      </button>
                    </div>
                    {analysisVerifyError ? (
                      <p style={{ margin: 0, fontSize: 12, color: "var(--danger-fg)" }}>{analysisVerifyError}</p>
                    ) : null}
                  </div>
                )}
            ...
            <div className="status-strip">
              ...
              <div className="sis-surface-sunken" style={{ padding: "12px 16px", minWidth: 280 }}>
                <p className="sis-label" style={{ marginBottom: 10 }}>Issue Log</p>
                <div className="issue-log-scroll" style={{ maxHeight: 168, overflowY: "auto", display: "flex", flexDirection: "column", gap: 6, paddingRight: 2 }}>
                  {issues.length === 0 ? (
                        ...
                        >
                          ...
                          {issue.fixType === "opening_clash" && (
                            <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
                              {readOnlyMode ? (
                                <span className="action-btn action-advisory">Read-only</span>
        ...
        {/* ── MAIN CONTENT AREA ── */}
        <div className="layout-main" style={{ minWidth: 0 }}>
          {/* 3D Viewer */}
          ...
          {/* Right sidebar */}
          <aside className="sidebar-stack" style={{ display: "flex", flexDirection: "column", gap: 16, minWidth: 0 }}>

            {/* Validation Summary */}
            <section className="sis-surface" style={{ padding: "20px 22px" }}>
              <div className="panel-title-row" style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 16 }}>
                <div>
            ...
            {/* Element Inspector */}
            <section className="sis-surface" style={{ padding: "20px 22px" }}>
              <div className="panel-title-row inspector-title-row" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 16 }}>
                <h2 style={{ fontSize: 15, fontWeight: 700, color: "var(--fg-primary)", margin: 0, letterSpacing: "-0.01em" }}>
                  Element Inspector
                </h2>
              ...
              {selectedWall ? (
                <div className="animate-enter inspector-grid">
                  {[
        ...
        {isHydrated ? (
          <MaterialPanel
            ...
            onFocusElement={handleReportElementFocus}
          />
        ) : (
          <section className={`sis-surface${darkMode ? " theme-dark" : ""}`} style={{ padding: "22px 24px" }}>
            <div className="calm-section-header">
              <div>
                <h2 className="calm-section-title">Material Analysis</h2>
                <p className="calm-section-subtitle">Preparing analysis workspace...</p>
              </div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[140, 100, 120].map((w, i) => (
                <div key={`material-panel-placeholder-${i}`} className="shimmer" style={{ height: 14, width: `${w}px` }} />
              ))}
            </div>
          </section>
        )}
        ...
        <section className="sis-surface" style={{ padding: "20px 22px" }}>
          ...
          <div
            className="analysis-grid optimization-scroll"
            style={{
              overflowY: optimizationActions.length > 1 ? "auto" : "visible",
              maxHeight: optimizationActions.length > 1 ? 360 : undefined,
              paddingRight: optimizationActions.length > 1 ? 2 : 0,
            }}
          >
            {optimizationActions.length === 0 ? (
